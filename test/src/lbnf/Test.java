...
public class Test {
	...
	public Test(String[] args) {
		try {
			...
			l = new LBNFLexer(new ANTLRInputStream(input));
			l.addErrorListener(new BNFCErrorListener());
		} catch (IOException e) {
			System.err.println("Error: File not found: " + args[0]);
			System.exit(1);
		}
		p = new LBNFParser(new CommonTokenStream(l));
		p.addErrorListener(new BNFCErrorListener());
	}

	public lbnf.Absyn.Grammar parse() throws Exception {
		/* The default parser is the first-defined entry point. */
		/* Other options are: */
		/*
		 * listDef, listItem, def, item, cat, label, labelId, profItem, intList,
		 * listInteger, listIntList, listProfItem, listString, listRHS, rHS,
		 * minimumSize, reg, listIdent
		 */
		LBNFParser.Start_GrammarContext pc = p.start_Grammar();
		
		lbnf.Absyn.Grammar ast = pc.result;
		System.out.println();
